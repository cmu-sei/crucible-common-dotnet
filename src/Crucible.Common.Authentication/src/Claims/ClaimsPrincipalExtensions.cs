...
namespace Crucible.Common.Authentication.Claims;
...
public static partial class ClaimsPrincipalExtensions
{
    ...
    /// are built from JWTs), return all values present. Values which are not arrays in the principal will be
    /// array-ified.
    /// </summary>
    ...
    /// <returns>An array of zero or more claim values at the specified path.</returns>
    public static string[] GetClaimValues(this ClaimsPrincipal principal, string roleClaimPath)
    {
        // we move the recursive execution into its own function for clarity
    ...
    private static string[] ExtractJsonClaimValues(string json, IEnumerable<string> pathSegments)
    {
        ...
        return [.. values.Where(v => v != null).Select(v => v!)];
    }
